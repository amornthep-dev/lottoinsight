...
import { Flame, Snowflake, Search, Calendar, Globe } from "lucide-react";
import hanoiData from "@/data/hanoi-lottery.json";
import {
  getDailyHotCold,
  getDayOfWeekPattern,
...
const data = hanoiData as DailyEntry[];

// ─── Countdown ───────────────────────────────────────────────────────────────
// Hanoi: every day at 18:10 ICT (UTC+7) = 11:10 UTC

function getNextDrawDate(): Date {
  const now = new Date();
  const ictMs = now.getTime() + 7 * 3600000;
  const ict = new Date(ictMs);
  const h = ict.getUTCHours();
  const m = ict.getUTCMinutes();
  const passedToday = h > 18 || (h === 18 && m >= 10);

  const next = new Date(ict.getTime());
  if (passedToday) {
    next.setUTCDate(next.getUTCDate() + 1);
  }
  next.setUTCHours(11, 10, 0, 0); // 18:10 ICT = 11:10 UTC
...
function Countdown() {
  ...
  useEffect(() => {
    function tick() {
      ...
      const ictDate = new Date(d.getTime() + 7 * 3600000);
      const days = ["อาทิตย์","จันทร์","อังคาร","พุธ","พฤหัสบดี","ศุกร์","เสาร์"];
      setNextStr(`${days[ictDate.getUTCDay()]} 18:10 น.`);
    }
    tick();
...
function EmptyState() {
  return (
    <div className="bg-[#1A1D2E] border border-[#2A2D3E] rounded-2xl p-10 text-center space-y-3">
      ...
      <p className="text-sm text-slate-600">ยังไม่มีข้อมูลผลหวยฮานอยในระบบ</p>
      <p className="text-xs text-slate-700">รันสคริปต์ fetch-hanoi.mjs เพื่อเพิ่มข้อมูลงวดแรก</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-[#1A1D2E] border border-[#2A2D3E] rounded-2xl p-5 space-y-4">
      ...
      <input
        ...
        placeholder={digit === 2 ? "เช่น  1 0" : "เช่น  5 2 3"}
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <section>
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-2xl font-bold text-slate-200">หวยฮานอย</h1>
          <span className="text-xs px-2 py-0.5 rounded-full bg-red-500/10 border border-red-500/30 text-red-400">ออกทุกวัน</span>
        </div>
        <p className="text-slate-500 text-sm">สถิติและวิเคราะห์หวยฮานอย — ออกผล 18:10 น.</p>
      </section>
          ...
          <div className="grid grid-cols-3 gap-px bg-[#2A2D3E]">
            {latest.prize1 && (
              <div className="bg-[#1A1D2E] px-3 py-4 text-center">
                <p className="text-[10px] text-slate-600 uppercase tracking-wider mb-1.5">รางวัลพิเศษ (5 ตัว)</p>
